...
def closest_university(location):
    ...
    minSch=""
    for row in csv_file:
        if(row['LATITUDE']!='-' and row['LONGITUDE']!='-' and distance(location, (float(row['LATITUDE']), float(row['LONGITUDE'])))<minDist):
            minDist=distance(location, (float(row['LATITUDE']), float(row['LONGITUDE'])))
            minSch=row['INSTNM']
    return minDist, minSch
